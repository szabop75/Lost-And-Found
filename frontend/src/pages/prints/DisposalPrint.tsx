...
export default function DisposalPrint() {
  ...
  return (
    <Box sx={{ p: 3, maxWidth: 900, mx: 'auto', fontSize: 14 }}>
      ...
      <Stack direction="row" alignItems="center" justifyContent="space-between" sx={{ mb: 2 }}>
        <Stack direction="row" alignItems="center" spacing={2}>
          ...
          <Box>
            ...
            <Typography variant="subtitle2">Selejtezési jegyzőkönyv</Typography>
          </Box>
        </Stack>
        <Box>
          <Typography variant="body2">Dátum: {disposalLog ? new Date(disposalLog.timestamp).toLocaleString('hu-HU') : (data?.foundAt ? new Date(data.foundAt).toLocaleDateString('hu-HU') : '')}</Typography>
      ...
      {/* Disposal section */}
      <Box sx={{ mb: 2 }}>
        <Typography variant="subtitle1" gutterBottom>Selejtezés</Typography>
        <Typography variant="body2">Selejtezést végző: {disposalLog?.actorUserId ?? '________________________'}</Typography>
        <Typography variant="body2">Időpont: {disposalLog ? new Date(disposalLog.timestamp).toLocaleString('hu-HU') : '________________________'}</Typography>
        <Typography variant="body2">Megjegyzés: {disposalLog?.notes ?? '-'}</Typography>
      </Box>
      ...
      <Stack direction="row" justifyContent="space-between" sx={{ mt: 6 }}>
        <Box sx={{ textAlign: 'center', width: '45%' }}>
          <Box sx={{ borderTop: '1px solid #000', pt: 1 }}>Selejtezést végző</Box>
        </Box>
        <Box sx={{ textAlign: 'center', width: '45%' }}>
          <Box sx={{ borderTop: '1px solid #000', pt: 1 }}>Tanú / Jóváhagyó</Box>
        </Box>
      </Stack>
